...
> R:Pone cantidad en cero inicializando un nuevo carrito, es importante llamarlo porque con Carrito c solo declaramos la variable pero no se inicializa, lo que no nos garantiza que el estado inicial de cantidad sea cero antes de empezar
...
> R:Linea 24, se obtuvo la mitad de lo esperado porque falta multiplicas el precio por la cantidad

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Multiplicar el precio por la cantidad para que nos de el valor esperado
...
> R:El operador incorrecto es <=, deberia ser un < ya que va de solo pueden ser 4 productos
...
> R:Las lineas 29 y 30, creo que me falta un test unitario para carrito con descuento
...
> R:un test unitario aisla una sola funcion y el de integracion prueba varias funciones, lo que tiene de bueno el unitario es que si falla sabes donde esta el problema
...
> R:En C, los arrays no tienen chequeo de límites en tiempo de ejecución
...
> R:Es mejor a la hora de leer codigo que no fue hecho por nosotros, si tenes que resolver un bug y no sabes donde esta, aislar las funciones sirve para saber cual esta fallando y cual no. Es decir, yo se lo que le estoy mandando a una funcion y cual es la salida esperada, si el codigo no cumple el test me lo deja saber
...
> R:El primer test no lo detecto porque  la cantidad es 1, por lo que quedo oculto el bug para total_precio_unitario
